...
func NewController(options ...ControllerOption) *Controller {
	service, err := service.MakeGeoServicer()
	if err != nil {
		fmt.Println(err)
	}
...
func (c *Controller) SearchAnswer(w http.ResponseWriter, r *http.Request) {
	...
	if authHeader == "" {
		newErrorResponce(w, fmt.Errorf("token is missing"), http.StatusForbidden)
		return
	}
	...
	status, address, err := c.Servicer.SearchAnswer(coordinates, token)
	if err != nil {
		newErrorResponce(w, err, status)
		return
	}
...
func (c *Controller) GeocodeAnswer(w http.ResponseWriter, r *http.Request) {
	...
	status, coords, err := c.Servicer.GeocodeAnswer(address, token)
	if err != nil {
		newErrorResponce(w, err, status)
		return
	}
